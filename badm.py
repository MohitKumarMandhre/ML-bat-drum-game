...
while True:
    ...
    if(isStarted == True):
        if(isOver == False):
            cv2.putText(frame,'YOUR SCORE IS -->> ' + str(score),(50,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2,cv2.LINE_AA)
        elif(isOver == True):
            cv2.putText(frame,'<<-- GAME OVER BUDDY -->>',(200,350),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),5,cv2.LINE_AA)
            cv2.putText(frame,'YOUR SCORE WAS -> ' + str(score),(300,380),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2,cv2.LINE_AA)
        ...
        time = 400
        cv2.putText(frame,'GAME WILL STARTS IN -->> ' + str(time - startingCounter),(200,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),3,cv2.LINE_AA)
        startingCounter = startingCounter + 1
        if startingCounter > time:
            isStarted = True

    cv2.imshow("MKM BADMINTON FRAME", frame)
    key = cv2.waitKey(1)
    if key == 27:
        break
